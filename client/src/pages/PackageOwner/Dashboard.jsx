...
const Dashboard = () => {
    ...
    const fetchDashboardData = async () => {
        try {
            const {data} = await axios.get('/api/bookings/existingPackage', {headers: { Authorization: `Bearer ${await getToken()}` }
            });
            if (data.success) {
  ...
  return (
    <div>
        <Title align='left' font='outfit' title='Dashboard' subtitle='Simplify your travel business: monitor packages, track bookings, and analyze revenue in real-time.'/>
        ...
        rounded-lg max-g-80 overflow-y-scroll'>
            <table className='w-full'>
                <thead className='bg-gray-50'>
                   <tr>
                    <th className='py-3 px-4 text-gray-800 font-medium'>User Name</th>
                    <th className='py-3 px-4 text-gray-800 font-medium
                    max-sm:hidden'>Package Name</th>
                    <th className='py-3 px-4 text-gray-800 font-medium
                ...
                <tbody className='text-sm'>
                    {dashboardData.bookings.map((item, index)=>(
                        <tr key={index}>
                            <td className='py-3 px-4 text-shadow-gray-700 border-t border-gray-300'>
                                {item.user.username}
                            </td>
